...
func.func @named_computation_in_manual_computation(%arg0: tensor<32xi32>) -> (tensor<32xi32>, tensor<32xi32>) {
  ...
  // CHECK-NEXT:   %7 = stablehlo.multiply %arg1, %arg1 {mhlo.sharding = "{devices=[2,4]<=[4,2]T(1,0) last_tile_dims={manual}}"} : tensor<8xi32>
  // CHECK-NEXT:   %8 = stablehlo.negate %arg1 {mhlo.sharding = "{devices=[1,4,2]<=[8] last_tile_dims={manual, replicated}}"} : tensor<8xi32>
  // CHECK-NEXT:   sdy.return {mhlo.sharding = "{}"} %7, %8 : tensor<8xi32>, tensor<8xi32>
  // CHECK-NEXT: } {mhlo.sharding = "{{[{][{]}}devices=[1,4,2]<=[8] last_tile_dims={manual, replicated}}, {devices=[1,4,2]<=[8] last_tile_dims={manual, replicated}}}", xla.sdy.manual_axes = #sdy<manual_axes{"a"}>} : (tensor<8xi32>) -> (tensor<8xi32>, tensor<8xi32>)
  // CHECK-NEXT: %3 = mhlo.copy %2#0 {mhlo.sharding = "{devices=[2,4]<=[4,2]T(1,0) last_tile_dims={manual}}"} : tensor<8xi32>
  // CHECK-NEXT: %4 = stablehlo.custom_call @SPMDShardToFullShape(%3) {mhlo.sharding = "{devices=[8]<=[8]}"} : (tensor<8xi32>) -> tensor<32xi32>
  // CHECK-NEXT: %5 = mhlo.copy %2#1 {mhlo.sharding = "{devices=[2,4]<=[4,2]T(1,0) last_tile_dims={manual}}"} : tensor<8xi32>
  // CHECK-NEXT: %6 = stablehlo.custom_call @SPMDShardToFullShape(%5) {mhlo.sharding = "{devices=[8]<=[8]}"} : (tensor<8xi32>) -> tensor<32xi32>
  // CHECK-NEXT: return %4, %6 : tensor<32xi32>, tensor<32xi32>
  %0:2 = sdy.manual_computation(%arg0) in_shardings=[<@mesh_0, [{"a", "b"}]>] out_shardings=[<@mesh_0, [{"a", "b"}]>, <@mesh_0, [{"a", "b"}]>] manual_axes={"a"} (%arg1: tensor<8xi32>) {
    ...
    sdy.return %1#0, %1#1 : tensor<8xi32>, tensor<8xi32>
  } : (tensor<32xi32>) -> (tensor<32xi32>, tensor<32xi32>)
  return %0#0, %0#1 : tensor<32xi32>, tensor<32xi32>
}
